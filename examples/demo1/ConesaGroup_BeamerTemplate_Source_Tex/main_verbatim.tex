...
% Package for TikZ graphics (for custom title frame)
\usepackage{tikz}
...
% Package for code listings - using verbatim instead of listings to avoid conflicts
\usepackage{verbatim}
\usepackage{fancyvrb}
\usepackage{xcolor}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    % Left color panel (45% width)
    \fill[conesaTeal] 
      (current page.north west) rectangle ([xshift=0.45\paperwidth]current page.south west);
  \end{tikzpicture}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Method 1}
        \begin{itemize}
          ...
          \item Step 3
        \end{itemize}
        \citationexample{Based on \citeauthor{Johnson2019} (\citeyear{Johnson2019})}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Method 2}
        \begin{itemize}
          ...
          \item Step C
        \end{itemize}
        \citationexample{Adapted from \citeauthor{Garcia2021} (\citeyear{Garcia2021})}
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
